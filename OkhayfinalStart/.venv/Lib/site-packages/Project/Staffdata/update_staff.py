...
class UpdateStaffScreen(QWidget):
    ...
    def create_personal_data_form(self):
        ...
        update_button.clicked.connect(self.update_staff_personal_data)
    ...
    def search_doctor(self):
        ...
        try:
            ...
            staff_data = search_staff_by_name(cnic)

            if staff_data:
                # Populate Personal Data
                self.first_name_input.setText(staff_data["FirstName"])
                self.last_name_input.setText(staff_data["LastName"])
                self.cnic_input.setText(staff_data["CNIC"])

                # Handle the joining date to ensure it's in the correct format
                if isinstance(staff_data["JoiningDate"], datetime.date):
                    joining_date_str = staff_data["JoiningDate"].strftime("%Y-%m-%d")
                    self.joining_date_input.setDate(QDate.fromString(joining_date_str, "yyyy-MM-dd"))
                else:
                    self.joining_date_input.setText(
                        staff_data["JoiningDate"])  # Assuming it's already a string in the correct format

                self.job_status_input.setText(staff_data["JobStatus"])

                # Populate Salary
                self.salary_amount_input.setText(str(staff_data["Amount"]))

                # Populate Address
                self.city_input.setText(staff_data["city"])
                self.street_input.setText(staff_data["street"])
                self.state_input.setText(staff_data["state"])
                self.postal_code_input.setText(staff_data["postal_code"])
                self.country_input.setText(staff_data["country"])

                # Populate Credentials
                self.username_input.setText(staff_data["Username"])
                self.password_input.setText(staff_data["PasswordHash"])

                QMessageBox.information(self, "Search Successful", "staff data loaded successfully.")
            else:
                QMessageBox.warning(self, "Search Failed", f"No staff found with CNIC: {cnic}")
        except Exception as e:
            # Handle unexpected errors
            QMessageBox.critical(self, "Error", f"An unexpected error occurred:\n{str(e)}")
            print(f"Error during search: {e}")

    def update_staff_personal_data(self):
        cnic = self.cnic_input.text()
        if not cnic:
        ...
        success = update_staff_personal_data(
            cnic=cnic,
    ...
    def update_salary(self):
        ...
        # Call backend function
        success = update_staff_salary(cnic=cnic, amount=salary_amount)
    ...
    def update_credentials(self):
        ...
        success = update_staff_credentials(cnic=cnic, username=username, password=password)
